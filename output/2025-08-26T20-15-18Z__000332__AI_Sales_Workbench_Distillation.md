...
- **Role in the argument**
  - **[SOURCE]** Orients readers to a cross‑functional initiative to unify GTM workflows and insights for field roles; establishes scope and collaborators. fileciteturn0file0
...
- **Role in the argument**
  - **[SOURCE]** Sets a unifying aspiration: a personalized workbench that consolidates data and workflows, evolves core motions (e.g., quoting) into a conversational, iterative, agent‑assisted experience, and names four top‑level success themes. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
- **Role in the argument**
  - **[SOURCE]** Grounds the case in “day‑in‑the‑life” research: tool sprawl, diluted strategy in execution, lack of proactive insights, lost technical validation signals, complexity in deal construction, inconsistent value storytelling, and value erosion in handoffs. The two *Life of a Seller* diagrams summarize activities, pain points, and impact across the funnel. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
### Strategy *(pp.5–6)*
- **Role in the argument**
  - **[SOURCE]** Commits to a unified, Salesforce‑embedded “single pane” with AI woven three ways: context‑sensitive assistance, intelligent summaries, and AI‑powered workflows; announces role‑tailored use‑case modules under a consolidated workspace. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
### Use Cases *(pp.6–7)*
- **Role in the argument**
  - **[SOURCE]** Enumerates core modules (e.g., unified dashboard/summary page, account research, quoting, proposal content, value‑based materials, renewals, sales plays, forecasting) and highlights *risk‑based filtering* to focus attention. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
- **Role in the argument**
  - **[SOURCE]** Puts adoption and productivity at the top; then maps KPIs to each module. An addendum proposes BHAG‑style outcomes (e.g., “tools sunset,” “searches go down,” increase in zero‑touch flows). fileciteturn0file0
- **What good looks like (DM rewrite)**
...
- **Role in the argument**
  - **[SOURCE]** States decision rules: modularity, ship‑and‑iterate, pilot with real deals, data‑over‑opinion, MECE clarity, integrate familiar tools, and nudge growth priorities. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
### Conclusion *(p.8)*
- **Role in the argument**
  - **[SOURCE]** Reframes the workbench as a strategic shift: unify, automate, personalize to improve how we acquire, grow, inspect, and run customer relationships. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
- **Role in the argument**
  - **[SOURCE]** Deconstructs selling to irreducible jobs, constraints (system friction, human limits, data entropy), and “must‑be‑true” conditions (automate, augment, make customer intent liquid) made feasible by LLMs and APIs. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
- **Role in the argument**
  - **[SOURCE]** Outlines the evolution from insights to an *agentic* flow where AI listens across channels, proposes options, the human curates, and teams iterate rapidly with lower risk. fileciteturn0file0
- **What good looks like (DM rewrite)**
...
- **End‑to‑end role study → problem codification.**
  - **[SOURCE]** Starts with “day in the life” evidence summarized visually, then clusters pains into 6–7 root themes that map to execution gaps. *(pp.3–4)* fileciteturn0file0
- **North Star sets the destination before the road.**
  - **[SOURCE]** Names a unified, intelligent workbench with a few measurable top‑line outcomes, anchoring all subsequent choices. *(p.2)* fileciteturn0file0
- **Architecture → AI design patterns → role‑tailored modules.**
  - **[SOURCE]** Commits to a consolidated surface (Salesforce‑embedded), then specifies three AI patterns (assist, summarize, automate), then lists use‑cases. *(pp.5–7)* fileciteturn0file0
- **Behavior shaping via risk‑focused prioritization.**
  - **[SOURCE]** The “Summary” page and risk filters focus attention on the right opportunities/accounts and actions. *(p.6)* fileciteturn0file0
- **Metrics attached to every module.**
  - **[SOURCE]** Puts adoption plus module‑level KPIs; extends with BHAG‑style outcome reductions (searches/time/tools). *(pp.7, 11)* fileciteturn0file0
- **Principles as trade‑off rules.**
  - **[SOURCE]** Uses explicit principles (MECE, data‑first, iterate) to resolve conflicts and guide sequencing. *(p.8)* fileciteturn0file0
- **Near‑term platform with a long‑term agentic arc.**
  - **[SOURCE]** Today: unified insights/workflows. Tomorrow: agent proposes; human curates; iterative loops increase velocity. *(p.10)* fileciteturn0file0
...
- **Purpose**
  ...
  - **[SOURCE]** Which “single place” moments are fragmented today, and what behaviors do we want to instill? *(Vision, Problems)* fileciteturn0file0
- **People**
  ...
  - **[SOURCE]** Where does intent or context get lost (systems, handoffs), and what signals would have prevented surprise? *(Problems)* fileciteturn0file0
- **Process**
  - **[SOURCE]** What parts can we *automate, augment, or structure* to reduce cognitive load? *(Appendix)* fileciteturn0file0
  - **[INFERRED]** What is the lightest integration path into today’s tools that still enables the desired behavior change?
- **Design Patterns**
  - **[SOURCE]** Which steps benefit from assistance, which from summarization, and which from automated workflow? *(Strategy)* fileciteturn0file0
- **Risk & Focus**
  - **[SOURCE]** What risk filters would most improve attention allocation for this role’s book of business? *(Summary page concept)* fileciteturn0file0
- **Metrics**
  - **[SOURCE]** What adoption, module‑level, and business KPIs prove this is working? *(How We Measure)* fileciteturn0file0
- **Principles**
  - **[SOURCE]** Which trade‑offs will we pre‑decide (e.g., iterate fast over breadth, data over opinion)? *(Guiding Principles)* fileciteturn0file0
- **Agentic Path**
  - **[SOURCE]** If the system listened across channels, what would it propose first, and what remains a human decision? *(Addendum)* fileciteturn0file0
...
- **Unify and embed; don’t re‑platform.**
  - **[SOURCE]** Consolidated workbench accessible via Salesforce; integrate familiar tools. *(pp.5–6, 8)* fileciteturn0file0
- **MECE clarity over surface area.**
  - **[SOURCE]** Information architecture must reduce confusion; “Summary” page concentrates signals. *(pp.6, 8)* fileciteturn0file0
- **Data integrity / liquidity.**
  - **[SOURCE]** Aim for accurate, integrated data; make unstructured intent capture and reuse possible. *(p.2, p.9)* fileciteturn0file0
- **AI woven in three modes.**
  - **[SOURCE]** Assist, summarize, automate — applied where they cut cognitive load and improve quality. *(p.5)* fileciteturn0file0
- **Iterative, pilot‑first delivery.**
  - **[SOURCE]** Ship early, pilot in real deals, let data drive decisions. *(p.8)* fileciteturn0file0
- **Role‑tailoring within a modular frame.**
  - **[SOURCE]** Use‑cases are adapted per persona/segment atop a shared platform. *(p.6)* fileciteturn0file0
- **Agentic end‑state direction.**
  - **[SOURCE]** Long‑term evolution to AI proposing options with human curation. *(p.10)* fileciteturn0file0
- **Outcome‑based measurement.**
  - **[SOURCE]** Adoption + module KPIs + BHAG “less searching, fewer tools.” *(pp.7, 11)* fileciteturn0file0
...
- **Keep**
  - **[SOURCE]** Structural spine: *North Star → problems → strategy (AI patterns) → role‑tailored use cases → metrics → principles → agentic future.* *(pp.2–10)* fileciteturn0file0
  - **[SOURCE]** “Summary page” with risk‑based focus and actions. *(p.6)* fileciteturn0file0
  - **[SOURCE]** Research‑backed problem statements from life‑of‑role studies. *(pp.3–4)* fileciteturn0file0
  - **[SOURCE]** Modularity + embed into familiar surfaces (Salesforce). *(pp.5, 8)* fileciteturn0file0
  - **[SOURCE]** KPI mapping per module and adoption metrics. *(p.7)* fileciteturn0file0
  - **[SOURCE]** Agentic evolution narrative (“AI listens, proposes; human curates”). *(p.10)* fileciteturn0file0
- **Avoid**
...
- **Decision rules**
  - **[SOURCE]** Apply the three AI patterns where they best reduce DM cognitive load; preserve modularity/embedded delivery. *(Strategy, Guiding Principles)* fileciteturn0file0
...
- **Decision rules**
  - **[SOURCE]** Maintain “Summary” as page one with role‑appropriate filters and recommended actions; define required data contracts for each module. *(p.6)* fileciteturn0file0
...
- **Decision rules**
  - **[SOURCE]** Mirror tiered KPI model (adoption → module KPIs → business outcomes → behavior proxies) and include BHAG‑style *reduction* targets (searches, tools, manual steps). *(pp.7, 11)* fileciteturn0file0

### Guiding Principles
- **Inputs**
  - **[INFERRED]** DM‑specific trade‑offs (e.g., standardization vs. regional nuance, central data control vs. local flexibility).
- **Decision rules**
  - **[SOURCE]** Keep MECE clarity, pilot in real teams, data‑over‑opinion gate for roadmap decisions; explicitly log exceptions. *(p.8)* fileciteturn0file0
...
- **Inputs**
  - **[SOURCE]** DM job decomposition (inspect, coach, allocate, escalate) and constraints: system friction, cognitive load, data entropy at the team level. *(p.9)* fileciteturn0file0
- **Decision rules**
...
- **Decision rules**
  - **[SOURCE]** Define propose‑→‑curate‑→‑iterate loops, including guardrails for sensitive actions (e.g., escalations). *(p.10)* fileciteturn0file0

---

## 7) Evidence Tagging *(where each idea anchors in the source)*

- **Context / Vision** — p.2 (“unified, intelligent…”, four North‑Star themes). **[SOURCE]** fileciteturn0file0
- **Problems** — pp.3–5 (two “Life of a Seller” diagrams; seven core problems including quoting, storytelling, handoffs). **[SOURCE]** fileciteturn0file0
- **Strategy & AI Patterns** — pp.5–6 (Salesforce‑embedded, assist/summarize/automate; role‑tailored use cases). **[SOURCE]** fileciteturn0file0
- **Summary Page & Risk Filters** — p.6 (path‑to‑plan, opportunities/accounts/tasks; attention via risk). **[SOURCE]** fileciteturn0file0
- **Metrics & BHAG notes** — pp.7, 11 (adoption + per‑module KPIs; tools sunset, searches/time reductions). **[SOURCE]** fileciteturn0file0
- **Guiding Principles** — p.8 (modularity, iterate, pilot, data‑first, MECE, familiarity, growth nudges). **[SOURCE]** fileciteturn0file0
- **First‑Principles** — p.9 (irreducible jobs, constraints, must‑be‑true, feasibility via LLM/APIs). **[SOURCE]** fileciteturn0file0
- **Agentic Co‑Pilot** — p.10 (listen across channels, propose options, human curator, iterative drafts). **[SOURCE]** fileciteturn0file0
...
- **Design at the level of *patterns*, not features.** Map assist/summarize/automate to DM jobs. **[SOURCE]** *(Strategy)* fileciteturn0file0
- **Instrument from day one.** Tie every module to adoption + outcome KPIs. **[SOURCE]** *(How We Measure)* fileciteturn0file0
- **Decide with principles.** Use the stated guardrails to resolve scope and sequencing. **[SOURCE]** *(Guiding Principles)* fileciteturn0file0
...
- **Role-tailoring on a modular platform**; **agentic path** over time.  fileciteturn0file0
...
> **Source section map to mirror tone/flow:** Context & Vision (p.2); Problems (pp.3–5); Strategy (pp.5–6); Use Cases + Summary concept (pp.6–7); Metrics (p.7 and ideation on p.11); Guiding Principles & Conclusion (p.8); First-Principles (p.9); Agentic Co-Pilot (p.10). Keep structure and cadence; **do not** copy sentences.  fileciteturn0file0
...
- Tag any interim assumptions with **[Assumption]** and any needed input with **[Input Needed]** so they’re easy to resolve later.  fileciteturn0file0
...
*This prompt preserves the structural spine and decision logic of the source while steering toward a DM-centric strategy doc—without copying content.*  fileciteturn0file0
