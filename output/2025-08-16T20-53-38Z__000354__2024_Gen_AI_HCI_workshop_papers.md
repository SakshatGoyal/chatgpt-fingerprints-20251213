...
**Background & Context**
- **Factual.** Addresses early‑stage product design where designers move between rough sketches and higher‑fidelity visuals; positions generative systems as helpers that respect sketch intent while exploring visual alternatives.
...
- **Factual.** Identifies three recurring challenges: interpreting abstract themes; one‑way generative iteration; and difficulty visualizing scenes. Authors argue for bidirectional tooling that supports back‑and‑forth between sketch and render.
...
- **Factual.** Calls for evaluations and designer‑in‑the‑loop controls that lock functional regions while styling aesthetics.
...
**Background & Context**
- **Factual.** Uses conceptual art to interrogate how generative systems redistribute agency, authorship, and process ownership in visual practice.
...
**Approach & Methods**
- **Factual.** Presents an interactive installation and a “5P”‑style analysis (problem, people, process, product, purpose) to examine shifting roles between artist and system.
...
**Findings & Contributions**
- **Factual.** Surfaces tensions between **process ownership** vs **product ownership** when artists operate “inside” model behaviors.
...
**Implications & Reflections**
- **Factual.** Argues that reframing the artist’s role clarifies where value accrues in model‑mediated practice.
...
**Background & Context**
- **Factual.** Investigates LLMs (ChatGPT, Gemini) as collaborators in research‑outline drafting, emphasizing process transparency.
...
**Approach & Methods**
- **Factual.** **Collaborative inquiry** with 4–6 researchers in weekly sessions; prompt strategies compared across models; reflections transcribed and analyzed.
- **Interpretive.** This is “meta‑prompting” as method: teams co‑design prompts, then audit the *dialogue* as much as the drafts.

**Findings & Contributions**
- **Factual.** Prompt variation strongly affects quality; Gemini seen as more proactive/justifying; ChatGPT as precise but prone to fabrication; both rely on explicit instructions; LaTeX formatting remains brittle.
...
**Implications & Reflections**
- **Factual.** Recommends disclosure, reviewer role clarity, and process‑oriented integration.
...
**Background & Context**
- **Factual.** Tackles the missing audio layer in UGC ecosystems by generating background music and SFX on‑the‑fly. Uses MusicGen/AudioGen, plus BLIP for image‑to‑text.
...
**Approach & Methods**
- **Factual.** Two game prototypes: (1) level creator → mood or BLIP caption → MusicGen; (2) vehicle builder → component summary or BLIP → AudioGen; ~4s latency per clip on consumer GPU.
...
**Findings & Contributions**
- **Factual.** Outputs fit aesthetics; quality hinges on prompt phrasing; image‑to‑text + audio worked better than expected; next steps: multi‑modal prompting and human‑in‑the‑loop editing.
...
**Implications & Reflections**
- **Factual.** Notes licensing/ethics and proposes expert‑in‑the‑loop workflows.
...
- **Factual.** Across papers, prompting is treated as *interaction design*: round‑trip pipelines (Inkspire), process/role transparency (Augmenting the Author), and prompt‑to‑audio chains for UGC show tangible patterns and limits (latency, fabrication, authorship).
...
**Background & Context**
- **Factual.** Argues that music composition is an ideal site to study human–AI teaming, with agency, authorship, and context at stake; proposes ethnographic study.
...
**Approach & Methods**
- **Factual.** Literature review + planned 18 interviews and ~60 hours of observation to capture situated practices.
- **Interpretive.** Treat fieldwork as requirement‑gathering for control metaphors (e.g., “arranger,” “session player,” “producer”).

**Findings & Contributions**
- **Factual.** Prior work shows steering tools increase control/trust; roles shift (curator/arranger vs co‑composer). Identifies gap: little in‑the‑wild study of compositional realities.
...
**Implications & Reflections**
- **Factual.** Calls for context‑sensitive roles with adaptable agency.
...
**Background & Context**
- **Factual.** Surveys choreography systems and reframes them via co‑creative interaction design across **ideation** (prep) and **studio** (prototyping) stages.
...
**Approach & Methods**
- **Factual.** Analyzes collaboration and communication styles using the COFI framework: turn‑taking vs parallel, reactive vs proactive, intentional vs consequential signals.
...
**Findings & Contributions**
- **Factual.** Three design needs: (1) parallel & spontaneous interaction, (2) complementary role division beyond generation, (3) richer two‑way communication (incl. implicit cues).
...
**Implications & Reflections**
- **Factual.** Highlights gaps in consequential communication and proactive AI in the studio.
...
**Background & Context**
- **Factual.** Maps artist options: **navigate** (prompt/sample), **adapt** (few‑shot/fine‑tune), **craft** (architecture/weight manipulation), arguing for tooling that restores process ownership.
...
**Approach & Methods**
- **Factual.** Synthesizes systems across the spectrum (e.g., semantic sliders, network blending) and outlines challenges/opportunities for model crafting tools.
- **Interpretive.** Treat models like materials (clay/ink): expose grain, resistance, and blendability.

**Findings & Contributions**
- **Factual.** Crafting supports authorship via process ownership; calls for tooling akin to “model sketching,” weight visualization, and direct manipulation.
- **Interpretive.** Roadmap: IDE‑quality environments for artistic ML, not just no‑code wrappers.

**Implications & Reflections**
- **Factual.** Advocates reframing evaluation—bias/overfit may be desirable as stylistic signature.
...
**Background & Context**
- **Factual.** Integrates music AI directly into the **DAW** via a MIDI plug‑in; supports non‑linear composition and **live improvisation** with anticipatory transformers.
- **Interpretive.** This is the “VST moment” for co‑creation: put AI where musicians already live, not in a detached sandbox.

**Approach & Methods**
- **Factual.** Two workflows: **co‑compose** by DAW infilling; **improvise** with chord‑conditioned anticipatory models adapted for latency and instrument control.
...
**Findings & Contributions**
- **Factual.** Shows DAW integration as enabler; presents dataset creation for lead‑sheet conditioning, latency handling, and instrument tokens.
- **Interpretive.** For adoption: minimize cognitive/context switches—AI should be routable like any other track.

**Implications & Reflections**
- **Factual.** Calls for domain‑expert co‑design and broader real‑time co‑performance research.
...
- **Factual.** Studies converge on **context‑native integration** (DAWs, studios) and **role clarity** (arranger, partner) while exposing the need for implicit, embodied cues in choreography. Empowering **model crafting** complements steering UIs.
...
**Background & Context**
- **Factual.** Points out a gap: 3D gen systems optimize for visual similarity, not for **physical viability** post‑fabrication.
- **Interpretive.** Treat physics as a *co‑author*: constraints protect feasibility and reduce prototyping waste.

**Approach & Methods**
- **Factual.** Frames two avenues: **preserve functionality** (segment & protect functional regions) and **encode functionality** (integrate material properties & simulation into generation). References prior work like Style2Fab.
- **Interpretive.** Pipeline pattern: *CAD semantics → constraint detection → guided stylization*.

**Findings & Contributions**
- **Factual.** Surfaces open problems: context‑dependent function, “smart” safe manipulations, aesthetics–structure coupling, material‑specific limits.
- **Interpretive.** UX should visualize *failure modes* early—stress heatmaps, min‑thickness warnings, printer/material presets.

**Implications & Reflections**
- **Factual.** Advocates feedback loops with simulation/testing in the generative process.
...
**Background & Context**
- **Factual.** Disabled‑led team explores GAI to make **embroiderable** patterns accessible—where detail/centering/line quality critically affect machinability.
- **Interpretive.** A vivid testbed for *utility‑constrained creativity*: images must be pretty *and* stitchable.

**Approach & Methods**
- **Factual.** Initial DALL‑E 2 trials → custom GPTs (few‑shot) to embed constraints (simple silhouettes, no texture, centered) → convert to SVG → embroider on two machines.
- **Interpretive.** “Constraint‑prompting” as design: codify craft rules directly into the model’s job.

**Findings & Contributions**
- **Factual.** Few‑shot customizations reduced some failure cases; persistent issues (ignored instructions like “no dots”) reveal brittle control & hidden defaults; overall increased inclusion and lowered effort for the end user.
- **Interpretive.** Product gap: **craft‑aware validators** and SVG‑native generators that honor geometric budgets (paths, jumps).

**Implications & Reflections**
- **Factual.** Plans quantitative evaluation and a dedicated accessible tool.
...
- **Factual.** Both papers turn **constraints** into **features**—physics for 3D printability and craft rules for embroidery—highlighting where standard T2X pipelines fail.
...
**Background & Context**
- **Factual.** Studies “small‑c” creativity in daily brainstorming with a tuned conversational LLM (brief, humble).
- **Interpretive.** Creativity emerges in the *back‑and‑forth*: the human frames, AI riffs, human prunes.

**Approach & Methods**
- **Factual.** One‑user dialogues in enterprise chat; examines divergent (generation) and convergent (summarize/classify) turns.
- **Interpretive.** Treat chat as a **joint whiteboard**: idea farming + list‑shaping in‑thread.

**Findings & Contributions**
- **Factual.** Shows fluid alternation between explore/optimize phases; AI both challenges and complies; “humble” tone supports critique.
- **Interpretive.** Design for *healthy friction*: nudge the AI to defend an idea before yielding; capture the reasoning.

**Implications & Reflections**
- **Factual.** Positions creativity as distributed across human+AI actions.
...
**Background & Context**
- **Factual.** Workshop with 21 students comparing **Self Ideation** vs **Co‑GPT Ideation** in rapid brainstorming (misinformation/hallucination topics).
- **Interpretive.** Quick studies of *how* people actually prompt under time pressure.

**Approach & Methods**
- **Factual.** Crazy‑8s‑style ideation, group clustering/ranking, prompt collection; quantitative/qualitative content analysis.
- **Interpretive.** Practical recipe to benchmark prompt behaviors and idea diversity.

**Findings & Contributions**
- **Factual.** Co‑GPT produced **~24% more unique ideas** and more top‑ranked items by participants; prompts were brief/straightforward; identified **consultant** vs **assistant** roles—and an **anti‑collaboration** (antagonistic) pattern.
- **Interpretive.** Ship **role presets** (consultant/assistant) and guardrails against antagonistic prompting that degrades outcomes.

**Implications & Reflections**
- **Factual.** LLMs augment breadth and some quality but don’t automatically surpass human ideas.
...
- **Factual.** Evidence supports **conversational scaffolding** for both divergence and convergence; role clarity and tone matter; more ideas isn’t automatically better.
...
**Background & Context**
- **Factual.** Re‑centers harms (representational/allocated) in AI‑as‑a‑Service contexts; critiques Western‑centric framings and documents stereotype amplification and erasure in T2I systems.
- **Interpretive.** Move beyond “is it biased?” to “**who is harmed, how, and where in the pipeline**?”

**Approach & Methods**
- **Factual.** Critical synthesis + concrete use‑case (e.g., prompts about class/family in Google Imagen 2) to illustrate quality‑of‑service gaps and erasure.
- **Interpretive.** Treat examples as **design probes** that reveal model positionality and enforcement asymmetries.

**Findings & Contributions**
- **Factual.** Highlights misalignment to global contexts; advocates **model positionality**, dataset documentation, and power‑aware development. Offers research questions for accountability.
- **Interpretive.** Practical lever: ship **harms‑first** diagnostics alongside fairness dashboards; include *community‑authored* tests.

**Implications & Reflections**
- **Factual.** Calls for transparency around data and decisions; community‑centric development.
...
**Background & Context**
- **Factual.** Examines whether direct, designed encounters with AI‑generated media can soften negative perceptions (details limited in the publicly parsed text).
...
**Approach & Methods**
- **Factual.** Workshop/experiment‑style evaluation (full procedural detail not extractable from the PDF text preview).
- **Interpretive.** Treat trials as “**moral UX**”: scaffold reflection, not persuasion; measure attitude shifts with transparency cues.

**Findings & Contributions**
- **Factual.** Paper targets perception change mechanisms via AI exposure (specific metrics/results unavailable in parsed text).
- **Interpretive.** Build patterns that *invite critique* during the experience (why this output, what data, what limits).

**Implications & Reflections**
- **Factual.** Positions experience design as part of harm mitigation discourse.
...
- **Factual.** The harms paper reframes bias into actionable **harm categories** and argues for positionality, documentation, and power‑aware practices; related work (auditing talk) stresses tooling for inspection; perception work points to experience design.
...
**Background & Context**
- **Factual.** Proposes a **rating scheme** to communicate data provenance/compliance for synthetic media pipelines (PDF available via external site; limited details accessible here).
...
**Approach & Methods**
- **Factual.** Framework‑driven proposal (full taxonomy/measures not extractable from the brief preview).
- **Interpretive.** Align with C2PA‑style manifests; expose chain‑of‑custody inside creative tools.

**Findings & Contributions**
- **Factual.** Articulates a scheme to rate/communicate compliance and provenance.
- **Interpretive.** Treat provenance as UX: defaults, warnings, and audience‑visible labels.

**Implications & Reflections**
- **Factual.** Encourages standardized signals across generation/editing.
...
**Background & Context**
- **Factual.** Examines how a mainstream tool’s **Generative Fill** aligns with or conflicts with user values and societal concerns.
- **Interpretive.** Everyday features are where ethics meets habit—nudges here scale.

**Approach & Methods**
- **Factual.** Value‑oriented analysis of workflows/outputs (arXiv extended abstract).
- **Interpretive.** Instrument value tensions (authenticity, consent, credit) as UI decisions.

**Findings & Contributions**
- **Factual.** Surfaces mismatches between creative goals and value impacts; argues for process‑level mitigations.
- **Interpretive.** Offer “ethical presets” (source‑bounded fill, watermark by default, consent‑aware content sourcing).

**Implications & Reflections**
- **Factual.** Suggests design changes to better reflect user/community values.
...
**Background & Context**
- **Factual.** Deploys generative music in a **participatory museum** piece, exploring audience co‑creation and institutional context.
- **Interpretive.** A lab‑in‑the‑wild for value questions: authorship, consent, and collective taste.

**Approach & Methods**
- **Factual.** Installation research: visitor inputs feed a generative pipeline; analysis of engagement and outcomes.
- **Interpretive.** Treat public installations as **ethnographic testbeds** for norms and expectations.

**Findings & Contributions**
- **Factual.** Documents how co‑creation shapes experience and reveals value tensions (e.g., representation, ownership).
- **Interpretive.** Instrument “credit sharing” and opt‑in data reuse in cultural settings.

**Implications & Reflections**
- **Factual.** Connects participatory design with generative pipelines.
...
**Background & Context**
- **Factual.** Theoretical reframing: posthumanism to interrogate human/non‑human boundaries in AI co‑creation.
...
**Approach & Methods**
- **Factual.** Conceptual analysis applied to creative practice.
- **Interpretive.** Translate theory into heuristics (agency mapping, obligation mapping, value articulation).

**Findings & Contributions**
- **Factual.** Surfaces ethical and identity implications of co‑creative assemblages.
- **Interpretive.** Encourage **assemblage thinking** in product roadmaps: whose goals, whose data, who benefits.

**Implications & Reflections**
- **Factual.** Positions posthumanism as lens for values‑centered design.
...
- **Factual.** Converges on **process‑level values**: provenance ratings, value‑sensitive feature audits, participatory deployments, and theoretical frames to guide design.
...
**Background & Context**
- **Factual.** Investigates whether linguistic nuance in model outputs improves **actionability** of insights. (Full text available; details beyond title not needed for core takeaway here.)
- **Interpretive.** Actionability ≠ verbosity: you need **just‑enough nuance** plus clear next steps.

**Approach & Methods**
- **Factual.** Comparative analysis of language variants and perceived usefulness (from the workshop paper).
- **Interpretive.** Treat this as copy‑design for AI: calibrate hedging, certainty, and specificity.

**Findings & Contributions**
- **Factual.** Points to a sweet spot where nuance supports decision‑making without diluting guidance.
- **Interpretive.** Product implication: **tone controllers** that bias the system toward “decide‑ready” outputs.

**Implications & Reflections**
- **Factual.** Recommends linguistic strategies for actionable communication.
...
**Background & Context**
- **Factual.** Articulates UX considerations specific to gen‑AI apps (from the workshop PDF; details in title‑level scope due to limited extraction).
- **Interpretive.** UX is the *governor*: controls, previews, provenance, and error recovery shape trust and satisfaction.

**Approach & Methods**
- **Factual.** Framework/heuristic‑style paper enumerating UX dimensions tied to generative specificity.
- **Interpretive.** Bake **iteration UX** (retries, variants, side‑by‑side) as first‑class, not add‑ons.

**Findings & Contributions**
- **Factual.** Distills patterns for steering, feedback, and failure handling.
- **Interpretive.** Offer *explainability without overload*: compact source hints + confident caveats.

**Implications & Reflections**
- **Factual.** Provides guidance to align UX with generative capabilities/limits.
- **Interpretive.** Treat **user control + provenance** as the core of “good” gen‑AI UX.

**Meta‑abstraction — Session 7 (Analysis)**
- **Factual.** Emphasizes **language calibration** and **UX patterns** for steerability and trust.
...
- **Factual (from slides).** The closing deck outlines session highlights and open questions; slides do not substitute for paper detail.
...
- **Tools *or* Partners.** Systems succeed when they enter existing habitats (DAWs, studios) and adopt clear roles (arranger, responder, improviser). **Design for role‑switching** over time and task.
- **Prompting as Interaction Design.** Move from ad‑hoc prompts to **prompting systems**: state capture → description → generation → critique → revision, with provenance and reversibility.
- **Creativity & Embodiment.** Text alone can’t carry embodied nuance; choreography and live performance need **implicit channels**, latency handling, and spontaneous initiative.
- **Trade‑offs & Constraints.** Encode physics, materials, and craft rules early; make constraint feedback legible and steerable, not punitive.
- **Harms & Values (Privacy/Synthetic Media).** Treat harms as concrete, **community‑articulated** categories; operationalize model positionality, provenance/rating schemes, and consentful defaults.
- **Emotion & Experience.** Tone and encounter design shape trust and perceived helpfulness; *humble* language and reflective experiences can improve collaboration and attitudes.

---

### What this means for a Creative Technologist (practice checklist)

1. **Integrate in‑context.** Ship plug‑ins/extensions where experts already work; minimize context switches.
2. **Encode constraints.** Add fabrication/craft validators and constraint‑aware generators; visualize failure modes pre‑commit.
3. **Design prompting systems.** Provide role presets (consultant/assistant/partner), rationale prompts, and critique loops.
4. **Make values visible.** Bundle provenance/rating labels, consent flows, and credit trails into media; default to ethical settings.
5. **Calibrate language.** Offer tone/actionability controls and “first‑step” suggestions; keep uncertainty honest.

---

## Sources (papers & slides cited)

Session 1: Inkspire; Augmenting the Author; UGC Audio; Conceptual Art/Equivalence.
Session 2: Music Composition (ethnographic plan); Choreography co‑creation; Model crafting; Live DAW improvisation.
Session 3: 3D fabrication constraints; Embroidery case study.
Session 4: Workplace conversational creativity; Rapid AIdeation.
Session 5: Harms; AI experience & perceptions (PDF limited); Auditing T2I (no PDF listed).
Session 6: Compliance rating (external PDF preview limited); Photoshop Generative Fill (values); Echo Chamber; Posthumanist AI.
Session 7: Nuanced language; UX of Gen‑AI Apps (limited extraction).
Closing slides.
