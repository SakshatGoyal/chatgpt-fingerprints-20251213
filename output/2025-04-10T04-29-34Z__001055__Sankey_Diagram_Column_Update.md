# Sankey Diagram Column Update

## You (2025-04-10T04:29:34.875529+00:00)

## Goal
You are given a complete Dash-based Python application that visualizes data using a Sankey chart, donut charts, and a data table. Your task is to surgically modify the code to replace the set of columns used in constructing the Sankey diagram. The old columns are to be completely removed from the logic, and a new set of six columns is to be used instead. Every other aspect of the application—functionality, layout, interactivity, download behavior, and aesthetic formatting—must remain strictly unchanged. The new Sankey must function identically to the original in terms of filtering, highlighting, and visual consistency.

## Return Format
Return a complete Python script that includes:
- Full modified code with old Sankey-related columns removed.
- New `STAGE_COLUMNS` list reflecting the updated Sankey dimensions.
- Proper integration of the new columns into label spacing and Sankey logic.
- The `CUSTOM_STAGE_LABELS` dict should be updated (or regenerated) to reflect the new columns used in the Sankey (you may use column names as default labels if no customization is needed).
- Ensure donut filters, download logic, and all dropdown filters still work without any side effects.

## Warnings & Guardrails
- Do NOT modify any logic related to donut charts, filtering behavior, UI styling, or layout geometry.
- The only required change is the substitution of the column set used in `STAGE_COLUMNS`.
- Do not introduce new features or UI controls.
- Make sure the Sankey still responds to filtering and preserves label centering, node/link color logic, and hover behavior.
- Assume all column names in the new set are present in the CSV file.
- Be explicit about replacing—not appending—the old columns.

## Context & Additional Elements
The new columns to use for the Sankey diagram are:
- `Regulatory Exposure`
- `Timing Dependency`
- `Market Dispersion`
- `Process Modularity`
...
The old columns being removed are:
- `Tension Axis`
- `Ambiguity Type`
- `Framing Move`
- `Friction Archetype`
...
These old columns were previously used for `STAGE_COLUMNS`, custom Sankey labels, and the default dropdown selection.

The updated code must:
- Replace `STAGE_COLUMNS` with the new list above.
- Ensure dropdown default values, Sankey rendering, and label generation use the new columns.
- Remove the old columns from any global or helper lists/constants.
- Leave all other components (especially donut-related logic) untouched.

You are acting as a senior Dash engineer conducting a high-fidelity column refactor for a production-grade interactive analytics dashboard. Deliver the full updated code.
...
"""
CSV_FILE_PATH = '/Users/sakshatgoyal/Desktop/Strategic Decision Making Work/Data Viz experiment/previous files/Tagging - Business Strategy with Clustering Labels.csv'
...
## ChatGPT (2025-04-10T04:33:18.132153+00:00)
...
# ------------------------------------------------------------------------------------
# NEW Sankey stage columns (old ones removed):
# ------------------------------------------------------------------------------------
STAGE_COLUMNS = [
    'Regulatory Exposure',
    'Timing Dependency',
...
# ------------------------------------------------------------------------------------
# Updated custom labels for the new Sankey columns
# (you may adjust to more descriptive text if desired):
# ------------------------------------------------------------------------------------
CUSTOM_STAGE_LABELS = {
    'Regulatory Exposure': 'Regulatory Exposure',
    'Timing Dependency': 'Timing Dependency',
    'Market Dispersion': 'Market Dispersion',
    'Process Modularity': 'Process Modularity',
    'Value Timeframe': 'Value Timeframe',
    'Knowledge Transferability': 'Knowledge Transferability'
}

# ------------------------------------------------------------------------------------
# Donut/filter-related columns remain unchanged:
# ------------------------------------------------------------------------------------
FILTER_COLUMNS = [
    'Regulatory Exposure',
    'Timing Dependency',
    ...
    for i in range(num_stages - 1):
        ...
        for _, row in merged.iterrows():
            ...
            if any_filter_active:
                ...
                # No filters => single set of links with default color
                links['source'].append(s_idx)
                links['target'].append(t_idx)
                links['value'].append(total_ct)
                links['label'].append(f"{s_val} -> {t_val}")
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Dropdown(
            id='stage-selection',
            options=[{'label': col, 'value': col} for col in STAGE_COLUMNS],
            value=STAGE_COLUMNS,  # Default to the new Sankey columns
            multi=True,
            style={'width': '600px'}
        )
    ], style={
...
# ------------------------------------------------------------------------------------
# CLIENTSIDE CALLBACK (placeholder)
# ------------------------------------------------------------------------------------
@app.callback(
    Output('window-width', 'data'),
    [Input('interval-resize', 'n_intervals')],
...
def update_window_size(_):
    """
    This is a placeholder for any clientside approach to read window.innerWidth.
    Returning a dummy fixed width here.
    """
    return 1200
...
def update_visuals(*args):
    ...
    # If user clicked Reset, ignore filter selections
    if triggered_id == 'reset-button':
        filter_values = {col: [] for col in ALL_FILTERABLE_COLUMNS}
    else:
        filter_values = {}
    ...
    # If fewer than 2 stages selected, return a blank Sankey
    if not selected_stages or len(selected_stages) < 2:
        no_sankey_fig = go.Figure()
        no_sankey_fig.update_layout(
            annotations=[dict(
                text="Please select at least 2 stages to display.",
                showarrow=False, x=0.5, y=0.5, xanchor='center', yanchor='middle'
            )]
        )
        return (
            no_sankey_fig,
            go.Figure(),
            "Insufficient stages selected.",
            [],
            [],
            "0 rows match current filters"
        )

    # Subset highlight DataFrame based on filters
    df_highlight = df_full.copy()
    for col, selected_vals in filter_values.items():
        if selected_vals:
            df_highlight = df_highlight[df_highlight[col].isin(selected_vals)]
    ...
    # Optional dynamic sizing based on window width
    sankey_height = (16/9) * window_width
    sankey_fig.update_layout(
        font=dict(family="'Anonymous Pro', monospace", size=FONT_SIZE),
        height=790,  # fixed or you can set to sankey_height
        margin=dict(l=160, r=160, t=40, b=160),
        hoverlabel=dict(font=dict(family="'Anonymous Pro', monospace", size=14))
    )
